...
const SettingsPage: React.FC<SettingsPageProps> = ({
  ...
  const handleSaveShift = (shiftToSave: Shift) => {
      ...
      setLocalWorkSettings(prev => ({ ...prev, shifts: updatedShifts }));
                ...
                <table className="w-full text-sm text-left text-gray-500 dark:text-slate-400">
                    ...
                    <tbody>
                        {statusItems.map((item) => {
                            ...
                            const unit = item.category === 'leave-day' ? ' gg' : item.category === 'leave-hours' ? ' ore' : '';
                            return (
                                <tr key={item.code} className="bg-white dark:bg-slate-800 border-b dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700/50">
                                    ...
                                    <td className="px-6 py-4 text-center font-semibold text-slate-800 dark:text-slate-200">{item.entitlement}{unit}</td>
                                    <td className="px-6 py-4 text-center font-semibold text-blue-600 dark:text-blue-400">{used}{unit}</td>
                                    <td className={`px-6 py-4 text-center font-bold ${balance < 0 ? 'text-red-500' : 'text-emerald-500'}`}>{balance}{unit}</td>
