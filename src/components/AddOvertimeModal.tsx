...
const HoursJustificationModal: React.FC<HoursJustificationModalProps> = ({ date, allLogs, allManualOvertime, statusItems, mode, onClose, onSave, onDelete }) => {
  ...
  // Filtra gli item in base alla modalità - USA CATEGORIE CORRETTE
  const filteredItems = mode === 'extra'
    ? statusItems.filter(item => item.category === 'overtime') // Straordinari, Corsi, etc.
    : statusItems.filter(item => item.category === 'leave-hours' || item.category === 'leave-day'); // Permessi, Malattia, etc.
  
  const overtimeItems = filteredItems;
  ...
  const handleSave = () => {
    ...
    const hrs = parseFloat(overtimeHours.replace(',', '.'));
    ...
    onSave(dateKey, durationMs, `code-${selectedOvertimeType.code}`, overtimeNote);
