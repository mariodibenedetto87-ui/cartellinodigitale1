        ...
        if (nfcSupported) {
            ...
            try {
                ...
                ndef.onreading = (event: any) => {
                    // Log dell'intero messaggio per debug
                    ...
                    for (const record of event.message.records) {
                        ...
                        if (record.recordType === "text") {
                            ...
                            // Metodo standard moderno: record.toText()
                            if (typeof record.toText === 'function') {
                                try {
                                    text = record.toText();
                                    console.log("Decoded via toText():", text);
                                } catch (e) {
                                    console.warn("Error using toText():", e);
                                }
                            }
                            ...
                            if (!text) {
                                ...
                                const status = data[0];
                                const encoding = status & 128 ? "utf-16" : "utf-8";
