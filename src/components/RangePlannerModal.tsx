...
import { StatusItem, ShiftType, LeaveType, WorkSettings } from '../types';
...
const RangePlannerModal: React.FC<RangePlannerModalProps> = ({ isOpen, startDate, statusItems, workSettings, onClose, onApply }) => {
  ...
  const [selectedValue, setSelectedValue] = useState<ShiftType | LeaveType>(workSettings.shifts[0]?.id || '');
  ...
  // Reset selected value when action type changes
  useEffect(() => {
    if (actionType === 'shift') {
      setSelectedValue(workSettings.shifts[0]?.id || '');
    } else if (actionType === 'leave') {
      const firstLeave = statusItems.find(i => i.category === 'leave-day' || i.category === 'leave-hours');
      setSelectedValue(firstLeave ? `code-${firstLeave.code}` : '');
    }
  }, [actionType, workSettings.shifts, statusItems]);
  ...
  const shiftOptions = workSettings.shifts.map(shift => ({ value: shift.id, label: shift.name }));
