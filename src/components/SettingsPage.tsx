...
import { StatusItem, Shift } from '../types';

import { calculateStatusUsage, calculateMonthlyStatusUsage } from '../utils/statusUtils';
...
const SettingsPage: React.FC = () => {
    ...
    const { allDayInfo, allManualOvertime, showToast } = useData();
    // useUI might be used for modals if we move them there, but for now they are local or specific to settings
    ...
    const [usageModalItem, setUsageModalItem] = useState<StatusItem | null>(null);
    const [monthlyUsageData, setMonthlyUsageData] = useState<number[]>([]);
    ...
    const usageData = React.useMemo(() => calculateStatusUsage(allDayInfo, currentYear, statusItems, allManualOvertime), [allDayInfo, currentYear, statusItems, allManualOvertime]);
    ...
    const handleSaveWorkSettings = () => {
        updateWorkSettings(localWorkSettings);
        showToast("Impostazioni orario di lavoro salvate!", 'success');
    };

    const handleSaveOfferSettings = () => {
        updateOfferSettings(localOfferSettings);
        showToast("Offerta speciale salvata!", 'success');
    };
    ...
    const handleSaveDashboardSettings = () => {
        updateDashboardLayout(localDashboardLayout);
        updateWidgetVisibility(localWidgetVisibility);
        showToast("Impostazioni dashboard salvate!", 'success');
    };
    ...
    const handleExportRotations = () => {
        ...
        if (savedRotations.length === 0) {
            showToast("Nessuna rotazione da esportare.", 'error');
            return;
        }
        ...
        reader.onload = (event) => {
            try {
                ...
                if (imported.rotations && Array.isArray(imported.rotations) && imported.rotations.every((item: any) => 'id' in item && 'name' in item && 'pattern' in item)) {
                    updateSavedRotations(imported.rotations);
                    if (imported.shifts && Array.isArray(imported.shifts)) {
                        updateWorkSettings({ ...workSettings, shifts: imported.shifts })
                    }
                    showToast("Rotazioni importate con successo!", 'success');
                } else {
                    throw new Error("Formato del file non valido");
                }
            } catch (error) {
                showToast("Errore: Il file non è un file di rotazioni valido.", 'error');
            } finally {
    ...
    const handleDeleteRotation = (id: string) => {
        if (window.confirm("Sei sicuro di voler eliminare questa rotazione?")) {
            ...
            updateSavedRotations(updatedRotations);
            showToast("Rotazione eliminata.", 'error');
        }
    };
    ...
    const handleDeleteStatus = (codeToDelete: number) => {
        if (window.confirm("Sei sicuro di voler eliminare questo status? L'azione è irreversibile.")) {
            ...
            updateStatusItems(updatedItems);
            showToast("Status eliminato.", 'error');
        }
    };
    ...
    const handleSaveStatus = (itemToSave: StatusItem) => {
        ...
        if (editingStatusItem && 'code' in editingStatusItem) {
            ...
            if (statusItems.some(item => item.code === itemToSave.code)) {
                showToast(`Il codice ${itemToSave.code} è già in uso.`, 'error');
                return;
            }
            updatedItems = [...statusItems, itemToSave];
        }

        updateStatusItems(updatedItems.sort((a, b) => a.code - b.code));
        showToast(`Status ${'code' in editingStatusItem! ? 'aggiornato' : 'aggiunto'} con successo.`, 'success');
        setIsStatusModalOpen(false);
        setEditingStatusItem(null);
    };

    const handleShowUsageDetails = (item: StatusItem) => {
        const usage = calculateMonthlyStatusUsage(allDayInfo, currentYear, item.code, statusItems, allManualOvertime);
        setMonthlyUsageData(usage);
    ...
    const handleDeleteShift = (shiftId: string) => {
        if (window.confirm("Sei sicuro di voler eliminare questo turno? L'azione è irreversibile.")) {
            ...
            updateWorkSettings(newWorkSettings);
            showToast("Turno eliminato.", 'error');
        }
    };
    ...
    const handleSaveShift = (shiftToSave: Shift) => {
        ...
        showToast(`Turno ${isEditing ? 'aggiornato' : 'aggiunto'} con successo.`, 'success');
            ...
            <div className="max-w-4xl mx-auto space-y-8">
                                ...
                                {statusItems.map((item) => {
                                    ...
                                    return (
                                        <tr key={item.code} className="bg-white dark:bg-slate-800 border-b dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700/50">
                                            ...
                                            <td
                                                className="px-6 py-4 cursor-pointer hover:text-teal-600 dark:hover:text-teal-400 transition-colors"
                                                onClick={() => handleShowUsageDetails(item)}
                ...
                <WorkLocationSettings
                    ...
                    onSave={(location) => {
                        ...
                        showToast('Posizione lavoro salvata!', 'success');
                    }}
                    onRemove={() => {
                        removeWorkLocation();
                        showToast('Posizione lavoro rimossa', 'success');
                    }}
                />

                <PushNotificationsSettings onShowToast={showToast} />
                    ...
                    isLayoutEditorOpen && (
                        <DashboardLayoutEditor
                            ...
                            onVisibilityChange={(widgetId, visible) => {
                                ...
                                updateWidgetVisibility(newVisibility);
                            }}
